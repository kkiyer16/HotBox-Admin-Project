...
class AdminLoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sh = getSharedPreferences("com.example.hotboxadmin", mode)
                ...
                if (ad_em.isEmpty() || ad_ps.isEmpty()) {
                    ...
                    fAuth.signInWithEmailAndPassword(ad_em, ad_ps)
                        .addOnCompleteListener {
                            if (ad_em != emailid) {
                                Toast.makeText(this, "Failed as you are not Admin", Toast.LENGTH_LONG).show()
                            } else {
                                if (it.isSuccessful) {
                                    Log.d("Main", "Successfully Logged in Admin: ${it.result?.user?.uid}")
                                    Toast.makeText(this, "Welcome to HotBox Admin Page!!", Toast.LENGTH_LONG).show()
